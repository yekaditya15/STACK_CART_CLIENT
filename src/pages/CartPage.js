...
const CartPage = () => {
  ...
  // delete item or decrement quantity
  const removeCartItem = (pid) => {
    try {
      let myCart = [...cart];
      let index = myCart.findIndex((item) => item._id === pid);
      if (index !== -1) {
        myCart.splice(index, 1); // Remove the entire product from the cart
        setCart(myCart);
  ...
  const decrementQuantity = (pid) => {
    try {
      ...
      let index = myCart.findIndex((item) => item._id === pid);
      if (index !== -1) {
        myCart[index].quantity = Math.max((myCart[index].quantity || 1) - 1, 0);
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get(
        API_BASE + "/api/v1/product/braintree/token"
      );
  ...
  const handlePayment = async () => {
    try {
      ...
      const { data } = await axios.post(
        API_BASE + "/api/v1/product/braintree/payment",
        {
          nonce,
          cart,
        }
      ...
      <div className="cart-page">
        <div className="row">
          <div className="col-md-12">
            <h1 className="text-center" style={{ color: "#232f3e" }}>
              {!auth?.user ? (
                ...
                <div className="row card flex-row" key={p._id}>
                  ...
                  <div className="col-md-4 cart-remove-btn">
                    <div className="quantity-container">
                      ...
                      <span className="quantity">{p.quantity || 1}</span>
                      <button
                        className="btn btn-secondary"
